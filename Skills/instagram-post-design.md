...
> **Project root:** `c:/Visual_posts/`. The autoloaded **[`CLAUDE.md`](../CLAUDE.md)** at the project root contains the trigger phrases and the Standard Post Workflow — read that for the *process*; read this skill for the *visual system*.
...
3. Then re-open `Ejemplos/` to re-anchor visually

## Brand handle

The Instagram handle that appears at the top of **every** slide is **`@lucianomusellaa`** (note the double "a"). This is the project owner. The reference creator `@ramiro.cubria` shown in the `Ejemplos/` images is only the *visual style* inspiration — never put their handle on a generated post.

Reference examples live in [Ejemplos/](../Ejemplos/) — always re-read them at the start of a new post job to ground myself visually. The username in those reference images is wrong for our purposes; ignore it and use `@lucianomusellaa`.
...
1. **Re-open `Ejemplos/`** and view at least 2 reference images to re-anchor visually.
...
4. **Generate the image(s).** I have **two complementary tools** available — pick the right one per slide:

   **A) Google Gemini "Nano Banana" image API** (preferred for hero visuals, illustrative covers, mockups, anything that needs real generated imagery)
   - Model: `gemini-2.5-flash-image-preview` (a.k.a. Nano Banana, Google's image generation model)
   - Endpoint: `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-image-preview:generateContent`
   - Auth: API key loaded from `../​.env` as `GEMINI_API_KEY` (never hardcode the key in scripts; never print it; never commit it).
   - Aspect ratio: pass `image_config: { aspect_ratio: "4:5" }` so the output natively matches Instagram portrait.
   - When prompting Nano Banana, **always include the full visual system from this skill in the prompt** (cream background, grid, Inter font, coral/yellow accents, headline rules, "Deslizá →", etc.) so the output stays on-brand.
   - Curl skeleton (run via Bash, reading the key from .env, never echoing it):
     ```bash
     source ../.env && curl -s "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-image-preview:generateContent?key=$GEMINI_API_KEY" \
       -H "Content-Type: application/json" \
       -d '{
         "contents": [{"parts":[{"text":"<FULL PROMPT HERE>"}]}],
         "generationConfig": {"responseModalities":["IMAGE"], "imageConfig":{"aspectRatio":"4:5"}}
       }'
     ```
   - The response returns base64 image data → decode and save to `Outputs/`.

   **B) HTML + headless render to PNG** (preferred for slides that are mostly typography, step slides, or anything where pixel-perfect text and brand colors matter — Nano Banana cannot reliably render exact text)
   ...
   - Example: `./render.sh Outputs/agentes-claude-code` renders every `.html` in that folder to a PNG of the same name.
   - Single-file mode also works: `./render.sh Outputs/agentes-claude-code/cover_v1.html`

   **Decision rule:** if the slide's main content is **text + a UI mockup**, use **HTML render (B)**. If the slide needs an **illustration, abstract visual, or photo-realistic mockup that doesn't exist**, use **Nano Banana (A)** to generate just that visual asset, save it to `Assets/`, then composite it inside the HTML template (B). This hybrid gives the best of both: pixel-perfect text + AI-generated imagery.
